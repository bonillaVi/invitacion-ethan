(() => {
  ...
  const RSVP_ENDPOINT = "https://script.google.com/macros/s/AKfycbzlbIujIzEOnI95ldq9NhWmmxQPRD9Cm5sWTO5PASvOo4ZEaSTApBNJmiWFDlPeIqA/exec";
  ...
  document.querySelectorAll(".v131-rsvp").forEach(section => {
    ...
    form.addEventListener("submit", async e => {
      ...
      if (!RSVP_ENDPOINT) {
        status.textContent = "Falta conectar Google Sheets en rsvp.js.";
        return;
      }
      ...
      try {
        /*
         * V153:
         * Google Apps Script responde mediante una redirección a googleusercontent.
         * En navegadores (especialmente desde localhost/Live Server) esa respuesta
         * puede quedar bloqueada por CORS aunque el POST sí llegue a la hoja.
         *
         * Para un RSVP no necesitamos leer datos privados de vuelta: únicamente
         * necesitamos entregar el registro. "no-cors" permite enviar el POST sin
         * que la redirección de Apps Script haga fallar la interfaz.
         */
        ...
        thanks.innerHTML = answer === "Asistiré"
          ? `<strong>¡Gracias por confirmar!</strong><span>Tu asistencia fue enviada. Nos dará mucho gusto compartir este día contigo.</span>`
          : `<strong>Gracias por avisarnos.</strong><span>Tu respuesta fue enviada correctamente.</span>`;
      } catch (err) {
        console.error("RSVP:", err);

        /*
         * Respaldo para navegadores que rechacen fetch keepalive.
         * sendBeacon no necesita leer una respuesta CORS.
         */
        try {
          const blob = new Blob(
          ...
          if (navigator.sendBeacon && navigator.sendBeacon(RSVP_ENDPOINT, blob)) {
            if (answer === "Asistiré") lanzarConfeti();
          ...
          console.error("RSVP beacon:", beaconError);
        }

        status.textContent = "No pudimos conectar con el registro. Revisa tu conexión e inténtalo nuevamente.";
